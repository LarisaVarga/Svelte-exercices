...
<div class:open={isMenuOpen}>
  ...
  <div class="burger-menu fixed top-2.5 right-2.5 flex md:hidden flex-col justify-between	w-7 h-6 cursor-pointer z-50" on:click={toggleMenu}>
    <div class="line w-full h-1 bg-black rounded-sm"></div>
    <div class="line w-full h-1 bg-black rounded-sm"></div>
    <div class="line w-full h-1 bg-black rounded-sm"></div>
  </div>

  <div class="menu fixed top-0 right-0 w-72 md:w-full h-full md:h-12 bg-white shadow-md z-40" transition:fly={{ x: isMenuOpen ? 0 : '100%', duration: 300 }}>
    <ul class="p-0 m-4 md:m-0 pt-6 md:pt-0 flex flex-col md:flex-row">
			<li class="menu-item mb-4 md:mb-0" aria-current={$page.url.pathname === '/' ? 'page' : undefined}>
				<a href="/">Home</a>
			</li>
